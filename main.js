...
function flattenForScan(tokens) {
...
var AUTO_SUBSET = [
  ...
  "diff",
  "graphql"
].filter((lang) => lowlight.registered(lang));
var CACHE_MAX = 128;
var cache = /* @__PURE__ */ new Map();
function highlightToLines(rawCode, language) {
  ...
  let tree;
  try {
    tree = language && lowlight.registered(language) ? lowlight.highlight(language, code) : lowlight.highlightAuto(code, { subset: AUTO_SUBSET });
  } catch {
    tree = { type: "root", children: [{ type: "text", value: code }] };
  }
  const spans = [];
  flatten(tree.children, void 0, spans);
  const lines = [[]];
...
var CodeHighlightPlugin = class extends Plugin {
  onload() {
